...
/*---[ Implement ]----------------------------------------------------------------------------------*/
 ...
 namespace TN3270 {
	...
	std::string	Local::Session::get(int baddr, size_t len, char lf) const {
	...
	std::string	Local::Session::get(int row, int col, size_t sz, char lf) const {

		std::lock_guard<std::mutex> lock(const_cast<Local::Session *>(this)->sync);

		lib3270_autoptr(char) text = lib3270_get_string_at(hSession, row, col, sz, lf);
